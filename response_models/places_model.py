...
class PlaceResponse(BaseModel):
    ...
    geometry_type: GeometryType | None = Field(
        default=None,
        description="Type of geometry. Null when geometry is not provided.",
        examples=["POINT", None],
    )

    geometry_data: GeometryData | None = Field(
        default=None,
        description=(
            "Coordinates matching geometry_type: "
            "a single [x, y] pair for POINT, "
            "or a list of [x, y] pairs for LINESTRING/POLYGON. "
            "Null when geometry is not provided."
        ),
        examples=[[[10.0, 20.0]], None],
    )
